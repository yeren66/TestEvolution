...
def related_files_between_commits(old_commit, new_commit, product_file_path, test_file_path):
...
def strategy_1(json_block, product_change_actions, test_change_actions):
    ...
    # 判断两个commit之间是否有其他commit修改了生产/测试相关的文件
    if related_files_between_commits(product_commit, test_commit, product_file_path, test_file_path):
        return 
...
def strategy_2(json_block):
    ...
    test_old_content = json_block["test_old_content"]
    test_new_content = json_block["test_new_content"]
    if related_files_between_commits(product_commit, test_commit, product_file_path, ""):
...
if __name__ == "__main__":
    input_dir = "/Users/mac/Desktop/TestEvolution/output"
    ...
    for json_block in data:
        ...
        command = f"./RefactoringMiner-3.0.4 -c {project_path} {test_commit} -json {refactoring_path}"
        try:
            result = subprocess.run(command, capture_output=True, text=True, check=True, shell=True)
            # return result.stdout.strip()
        except subprocess.CalledProcessError as e:
        ...
        if tag == "negative":
            strategy_1(json_block, product_change_actions, test_change_actions)
        else:
            strategy_2(json_block)
