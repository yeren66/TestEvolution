...
class Config(object):
    ...
    def __init__(self): 
        ...
        self.n_gpu = 1
        self.decoder_path = 'Salesforce/codet5-base'
    ...
    for line in data:
        examples.append(
            Example(
                ...
                test_tgt = repr(line['test_tgt'])[1:-1]
            )
        )
        idx += 1
    return examples
...
def convert_examples_to_features(examples, args):
    ...
    for example_index, example in enumerate(tqdm(examples,total=len(examples))):
        ...
        edit_attention_mask = []
        edit_input_ids_1 = []
        edit_attention_mask_1 = []
        edit_lst = []
        for i in edit_seq:
            edit_lst.extend(i)
        edit_lst = ' '.join(edit_lst)
        encoded_tmp_1 = args.tokenizer(edit_lst, padding=True, truncation=True,max_length=args.max_edit_seq_length, return_tensors='pt')
        edit_input_ids.extend(np.array(encoded_tmp_1['input_ids']).flatten())
        edit_attention_mask.extend(np.array(encoded_tmp_1['attention_mask']).flatten())
        ...
        for item in edit_seq:
            ...
            edit_input_ids_1.extend(np.array(encoded_tmp['input_ids']).flatten())
            edit_attention_mask_1.extend(np.array(encoded_tmp['attention_mask']).flatten())
